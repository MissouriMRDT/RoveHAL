...
/////////////////////////////////
#include "RoveCcp.h"
#include "..\RoveTimerInterrupt\RoveTimer.h"
...
namespace roveware
{
  ...
  void ccpTicksIsr( struct CcpTicks* CcpTicks )
  {
    ...
    TimerPrescaleSet( CcpTicks->Timer.Hw.TIMER_BASE_ADDRESS,
          ...
          // (uint8_t)( CcpTicks->Timer.period_ticks >> 16 ) ); todo
          (uint8_t)( 0x00FFFFFF >> 16 ) );

    TimerLoadSet(     CcpTicks->Timer.Hw.TIMER_BASE_ADDRESS,
                      CcpTicks->Timer.Hw.TIMER_CHANNEL_AB,
          //        ( CcpTicks->Timer.period_ticks  & 0xFFFF ) );  todo
           (uint8_t)( 0x00FFFFFF  & 0xFFFF ) );

    uint32_t capture_ticks = TimerValueGet24( CcpTicks->Timer.Hw.TIMER_BASE_ADDRESS,
                                              CcpTicks->Timer.Hw.TIMER_CHANNEL_AB );

    bool digital_read      = GPIOPinRead(     CcpTicks->Hw.PORT_BASE_ADDRESS,
                                              CcpTicks->Hw.PIN_BIT_MASK ) && true;

    if(   ( digital_read == HIGH )
      and ( digital_read != CcpTicks->last_digital_read ) )
    {
      CcpTicks->PulseWidths.pushToBack(   capture_ticks );
      CcpTicks->PulsePeriods.pushToBack(  capture_ticks + CcpTicks->last_capture_ticks ); 
      ccpEdgeIsCaptured(                  CcpTicks->timer, true ); 
    }

    CcpTicks->last_capture_ticks = capture_ticks;
    CcpTicks->last_digital_read  = digital_read;
