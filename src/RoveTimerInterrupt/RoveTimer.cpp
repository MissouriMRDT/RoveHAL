...
{ 
  if (      TIMER_CHANNEL_AB == TIMER_A ) {
    return (uint32_t)( HWREG( TIMER_BASE_ADDRESS + TIMER_O_TAR ) 
                   + ( HWREG( TIMER_BASE_ADDRESS + TIMER_O_TAPS ) << 16 ) ); }

  else if ( TIMER_CHANNEL_AB == TIMER_B ) { 
    return (uint32_t)( HWREG( TIMER_BASE_ADDRESS + TIMER_O_TBR ) 
                   + ( HWREG( TIMER_BASE_ADDRESS + TIMER_O_TBPS ) << 16 ) ); }
}
